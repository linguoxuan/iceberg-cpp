...
Result<std::shared_ptr<Table>> Transaction::Commit() {
  ...
  if (!CanRetry()) {
    ...
    if (!commit_result.has_value() &&
        commit_result.error().kind == ErrorKind::kPendingUpdateFailed) {
      commit_result = CommitFailed(std::move(commit_result.error().message));
    }
  }
  ...
  if (ctx_->metadata_builder->base() != ctx_->table->metadata().get()) {
    ...
    for (const auto& update : pending_updates_) {
      ...
      if (!commit_status.has_value()) {
        // Wrap as kPendingUpdateFailed to break out of the retry loop.
        return PendingUpdateFailed(std::move(commit_status.error().message));
      }
    }
  }
